            ...
            if (bookingData.isSlotBooking) {
              BookingServiceStep1(
                data: ServiceDetailResponse(
                    serviceDetail:
                        (await getServiceDetails(serviceId: bookingData.serviceId.validate(), customerId: appStore.userId, fromBooking: true))
                            .serviceDetail),
          ...
          Row(
            ...
            children: [
              if (bookingData.isPackageBooking && bookingData.bookingPackage != null)
                CachedImageWidget(
                  url: bookingData.bookingPackage!.imageAttachments.validate().isNotEmpty
                      ? bookingData.bookingPackage!.imageAttachments.validate().first.validate()
                      : "",
                  height: 80,
                  width: 80,
                  fit: BoxFit.cover,
              ...
              children: [
                ...
                if (bookingData.paymentStatus != null &&
                    (bookingData.status == BookingStatusKeys.complete ||
                        bookingData.paymentStatus == SERVICE_PAYMENT_STATUS_ADVANCE_PAID ||
                        bookingData.paymentStatus == SERVICE_PAYMENT_STATUS_PAID))
                  Column(
                    children: [
                      Divider(height: 0),
                      Row(
