    ...
    await flutterWave.charge().then((value) {
      if (value.status == "successful") {
        ...
        verifyPayment(transactionId: value.transactionId.validate(), flutterWaveSecretKey: flutterWaveSecretKey).then((v) {
          if (v.status == "success") {
            savePay(data: bookDetailData, paymentMethod: PAYMENT_METHOD_FLUTTER_WAVE, paymentStatus: SERVICE_PAYMENT_STATUS_PAID, txnId: value.transactionId.validate(), totalAmount: totalAmount)
                .catchError(onError);
          } else {
